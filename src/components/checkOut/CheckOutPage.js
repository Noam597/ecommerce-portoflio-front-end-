...
const CheckOutPage = () => {
  ...
  const sendQuick = () => {
    ...
    fastDelivery
      ? setTotal((prevTotal) => prevTotal - 80)
      : setTotal((prevTotal) => prevTotal + 80);
  };

  const submit = (e) => {
    e.preventDefault();
    const { userName, lastName, phone, email, address } = deliveryForm;
    if (
      userName.length === 0 ||
      lastName.length === 0 ||
      phone.length === 0 ||
  ...
  return (
    <section style={screenStyle}>
      <Button
        text="Back to Cart"
        onClick={() => navigate("/ecommerce-portoflio-front-end-/cart")}
      />
      {sent ? (
        ...
        <div className={styles.checkOut}>
          ...
          <div className={styles.delivery}>
            <Form>
              ...
              />
              {errors.userName && (
                <h5 className={styles.error}> * {errors.userName}</h5>
              )}
              <br />
              <h3>Last Name:</h3>
              <Input
              ...
              />
              {errors.lastName && (
                <h5 className={styles.error}> * {errors.lastName}</h5>
              )}
              <br />
              <h3>Phone:</h3>
              <Input
              ...
              />
              {errors.phone && (
                <h5 className={styles.error}> * {errors.phone}</h5>
              )}
              <h3>Email:</h3>
              <Input
                name="email"
              ...
              />
              {errors.email && (
                <h5 className={styles.error}> * {errors.email}</h5>
              )}
              <h3>address:</h3>
              <Input
                name="address"
              ...
              />
              {errors.address && (
                <h5 className={styles.error}> * {errors.address}</h5>
              )}
              <h4>
                {" "}
                <input
